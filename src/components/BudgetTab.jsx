...
export default function BudgetTab({ tripId, budget, expenses, setExpenses }) {
  ...
  return (
    <>
      <style>{`
        .budget-stats { grid-template-columns: repeat(3, 1fr); }
        .budget-form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; }
        .expense-row { flex-direction: row; align-items: center; }
        .expense-name { font-size: 13px; }
        @media (max-width: 480px) {
          .budget-stats { grid-template-columns: 1fr !important; }
          .budget-form-grid { grid-template-columns: 1fr !important; }
          .expense-row { flex-direction: row; flex-wrap: wrap; gap: 8px; }
          .expense-name { font-size: 12px; }
        }
      `}</style>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{
          ...
          <div className="budget-stats" style={{ display: "grid", gap: 10, marginBottom: 20 }}>
            {[
          ...
          <div style={{ marginBottom: 6 }}>
            ...
            <div style={{
              background: "#13121f", borderRadius: 99, height: 8,
              overflow: "hidden", border: "1px solid rgba(255,255,255,0.05)"
            }}>
              <div style={{
                height: "100%", borderRadius: 99,
                background: isOver ? "#f87171" : percentage > 80 ? "#fbbf24" : "linear-gradient(90deg, #6366f1, #8b5cf6)",
                width: `${percentage}%`, transition: "width 0.4s ease"
              }} />
            </div>
            {isOver && (
